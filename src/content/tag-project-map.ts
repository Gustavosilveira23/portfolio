// Maps service tag text → project slug
// Includes both EN and PT variants for bilingual support
export const tagProjectMap: Record<string, string> = {
  // Design to Code
  "Figma to code": "duo-ai",
  "Claude Code": "raio-x-creator",

  // Product Design
  "End-to-end design": "softruck-redesign",
  "Design end-to-end": "softruck-redesign",

  // Research & Strategy
  "User research": "hotmart-research-repository",
  "Pesquisa com usuários": "hotmart-research-repository",

  // Design Systems
  "Design tokens": "raio-x-creator",

  // AI Product Design
  "AI-native interfaces": "duo-ai",
  "Interfaces AI-native": "duo-ai",
};
